...
def experiment_folds(st, reg, x, y):
    ...
    for i in inter:
        ...
        models = {
            i: [np.mean(mean_errors), np.mean(mean_scores), reg]
        }

    models = sorted(models.items(), key=itemgetter(1))
    
    #return np.mean(mean_scores), np.mean(mean_errors), models[0][1][1]
    return models[0][1][1], models[0][1][0], models[0][1][2]
